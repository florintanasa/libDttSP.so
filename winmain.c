...
//		WriteFile (top.meas.spec.fd, (LPVOID) & uni.spec.label, sizeof (int), &NumBytesWritten, NULL);
			write(top.meas.spec.fd,(LPVOID) & uni.spec.label, sizeof (int));
//		WriteFile (top.meas.spec.fd, (LPVOID) uni.spec.output, sizeof (float) * uni.spec.size, &NumBytesWritten, NULL);
			write(top.meas.spec.fd, (LPVOID) uni.spec.output, sizeof (float) * uni.spec.size);
...
//		WriteFile (top.meas.mtr.fd, (LPVOID) & uni.meter.label, sizeof (int), &NumBytesWritten, NULL);
			write(top.meas.mtr.fd, (LPVOID) & uni.meter.label, sizeof (int));
//		WriteFile (top.meas.mtr.fd, (LPVOID) & uni.meter.snap.rx, sizeof (REAL) * MAXRX * RXMETERPTS, &NumBytesWritten, NULL);
			write(top.meas.mtr.fd, (LPVOID) & uni.meter.snap.rx,sizeof (REAL) * MAXRX * RXMETERPTS);
//		WriteFile (top.meas.mtr.fd, (LPVOID) & uni.meter.snap.tx, sizeof (REAL) * TXMETERPTS, &NumBytesWritten, NULL);
			write (top.meas.mtr.fd, (LPVOID) & uni.meter.snap.tx, sizeof (REAL) * TXMETERPTS);
		sem_post (&top.sync.upd.sem);
    }
	pthread_exit (0);
}
...
*/
top.parm.fd = mkfifo(top.parm.path, 0666); // create fifo SDRcommands+pid
...
/*
 ...
 //     DisplayErrorText (GetLastError ());
    ...
    {
*/
      top.meas.mtr.fd = mkfifo(top.meas.mtr.path, 0666); // create fifo /dev/shm/SDRmeter
      sem_post (&setup_update_sem);
/*
...
/*
 ...
 //     DisplayErrorText (GetLastError ());
    ...
    {
*/
      top.meas.spec.fd = mkfifo(top.meas.spec.path, 0666);//create fifo SDRspectrum+pid
      sem_post (&setup_update_sem);
 /*
